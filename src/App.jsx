...
const App = () => {
  ...
  const fetchYahooPrice = async (symbol) => {
    try {
      const res = await fetch(`/yahoo/v8/finance/chart/${symbol}`);
      const data = await res.json();
  ...
  const handleFetchHistoricalPrice = async (dateStr, symbol, type) => {
    ...
    try {
      ...
      if (type === 'Crypto') {
        ...
        const res = await fetch(`/yahoo/v8/finance/chart/${sym}?interval=1d&period1=${startTimestamp}&period2=${endTimestamp}`);
        const data = await res.json();
